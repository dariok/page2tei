...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xd:doc>
        <xd:desc>Page XML file by which program created. Value: OCRD or Transkribus 'PAGEXML' with which ID definition is referenced in the mets:fileGrp to the PAGE xml files.</xd:desc>
    </xd:doc>
    <xsl:param name="PAGEprogram"/>
    ...
    <xsl:template match="/mets:mets">
        <TEI>
            ...
            <xsl:if test="not($debug)">
                <facsimile>
                    <xsl:choose>
                        <xsl:when test="$PAGEprogram = 'OCRD'">
                            <xsl:apply-templates
                                select="mets:fileSec//mets:fileGrp[@USE = $PAGEXML]/mets:file"
                                mode="facsimile"/>
                        </xsl:when>
                        <xsl:when test="$PAGEprogram = 'Transkribus'">
                            <xsl:apply-templates
                                select="mets:fileSec//mets:fileGrp[@ID = $PAGEXML]/mets:file"
                                mode="facsimile"/>
                        </xsl:when>
                    </xsl:choose>
                </facsimile>
    ...
    <xsl:template match="p:Page | pc:Page" mode="facsimile">
        ...
        <xsl:variable name="coords" select="tokenize(p:PrintSpace/p:Coords/@points | pc:PrintSpace/pc:Coords/@points, ' ')"/>
        ...
        <surface ulx="0" uly="0" lrx="{@imageWidth}" lry="{@imageHeight}" xml:id="facs_{$numCurr}">
            ...
            <xsl:apply-templates
                select="p:PrintSpace | p:TextRegion | p:SeparatorRegion | p:GraphicRegion | p:TableRegion | pc:PrintSpace | pc:TextRegion | pc:SeparatorRegion | pc:GraphicRegion | pc:TableRegion"
                mode="facsimile"/>
        </surface>
    </xsl:template>
